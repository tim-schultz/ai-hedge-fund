...
MOCK_SLOT0_OUTPUT = (
    b"\x00" * 224  # 7 static values × 32 bytes
)
...
def test_decode_slot0_outputs_success():
    ...
    valid_output = encode(["(bool,bytes)[]"], [[
...
def test_decode_slot0_outputs_failed_call():
    ...
    failed_output = encode(["(bool,bytes)[]"], [[
        (False, b""),
        (False, b"")
    ]])
...
def test_decode_slot0_outputs_insufficient_data():
    ...
    insufficient_output = encode(["(bool,bytes)[]"], [[
        (True, b"\x00" * 100),
        (True, b"\x00" * 100)
    ]])
...
def test_setup_directories(tmp_path):
    ...
    with patch('swap_data.POOLS_DIR', tmp_path / "pools"), \
...
@pytest.mark.parametrize("block_stride,chunk_size", [
...
def test_full_data_fetching_flow(tmp_path, mock_web3):
    ...
    with patch('swap_data.Web3', return_value=mock_web3), \
