...
def main():
    if not CHUNK_DIR.exists():
        raise FileNotFoundError(f"{CHUNK_DIR} not found. Run swap_data.py first to build slot0 chunks.")
    ...
    long_df = df.reset_index().melt(id_vars="block_number", var_name="pool", value_name="sqrtPriceX96").dropna(subset=["sqrtPriceX96"])
    ...
    print(f"Wrote interactive HTML plot to {out_path} with " f"{len(long_df['pool'].unique())} pools.")
