...
from src.domain_types import OrderSide, Score, Ticker, ValuationOutput
from src.utils.api_key import get_api_key_from_state
from src.utils.progress import progress


class ValuationAgent(BaseAnalysisAgent):
    """Valuation analysis agent using multiple valuation methodologies."""

    @property
    def name(self) -> str:
        return "Valuation Analyst"
    ...
    def analyze(self, state: AgentState) -> Dict[str, Any]:
        ...
        return valuation_analysis(state, self)


def valuation_analysis(state: AgentState, agent: ValuationAgent) -> Dict[str, Any]:
    """Run valuation across tickers and write signals back to `state`."""
    data = state["data"]
    end_date = data["end_date"]
    tickers: List[Ticker] = data["tickers"]
    api_key = get_api_key_from_state(state, "FINANCIAL_DATASETS_API_KEY")
    valuation_results: Dict[Ticker, ValuationOutput] = {}

    for ticker in tickers:
        progress.update_status(agent.name, ticker, "Fetching financial data")
        ...
        if not financial_metrics:
            progress.update_status(agent.name, ticker, "Failed: No financial metrics found")
            continue
        most_recent_metrics = financial_metrics[0]

        # --- Enhanced line‑items ---
        progress.update_status(agent.name, ticker, "Gathering comprehensive line items")
        ...
        if len(line_items) < 2:
            progress.update_status(agent.name, ticker, "Failed: Insufficient financial line items")
            continue
        ...
        progress.update_status(agent.name, ticker, "Calculating WACC and enhanced DCF")
        ...
        if not market_cap:
            progress.update_status(agent.name, ticker, "Failed: Market cap unavailable")
            continue
        ...
        if total_weight == 0:
            progress.update_status(agent.name, ticker, "Failed: All valuation methods zero")
            continue
        ...
        # Create properly typed valuation output
        valuation_output: ValuationOutput = {
            "ticker": ticker,
            "intrinsic_value": dcf_val if dcf_val > 0 else owner_val,
            "current_price": market_cap,
            "margin_of_safety": weighted_gap * 100,  # Convert to percentage
            "valuation_method": "Multi-method DCF + Owner Earnings",
            "confidence_level": confidence / 100.0,  # Convert to 0-1 scale
            "assumptions": {
                "dcf_value": dcf_val,
        ...
        valuation_results[ticker] = valuation_output

        # Also store in old format for compatibility
        valuation_analysis[ticker] = agent.format_signals(
            ticker=ticker,
            signals={
                "intrinsic_value": valuation_output["intrinsic_value"],
                "current_price": valuation_output["current_price"],
                "margin_of_safety": valuation_output["margin_of_safety"],
                "valuation_methods": reasoning,
            },
            recommendation=signal,
            confidence=confidence / 100.0,
        )

        progress.update_status(agent.name, ticker, "Done", analysis=json.dumps(reasoning, indent=4))

    # ---- Emit message (for LLM tool chain) ----
    msg = HumanMessage(content=json.dumps(valuation_analysis), name=agent.name)
    if state["metadata"].get("show_reasoning"):
        show_agent_reasoning(valuation_analysis, "Valuation Analysis Agent")

    # Add the signal to the analyst_signals list
    state["data"]["analyst_signals"][agent.name] = valuation_analysis
...
def calculate_enhanced_dcf_value(fcf_history: list[float], growth_metrics: Dict[str, Any], wacc: float, market_cap: float, revenue_growth: float | None = None) -> float:
...
def calculate_dcf_scenarios(fcf_history: list[float], growth_metrics: Dict[str, Any], wacc: float, market_cap: float, revenue_growth: float | None = None) -> dict:
