...
def plot_synthetic_paths(debug_dir_path="src/selv/debug", save_path=None, figsize=(12, 8), alpha=0.6):
    ...
    for (strategy, path_id), g in mc.groupby(["strategy", "path_id"]):
        ax.plot(g["datetime"], g["close"], alpha=alpha, linewidth=0.8, label=f"{strategy} | id {path_id}")
...
def analyze_monte_carlo_results(results_path="mc_results.csv"):
    ...
    summary = mc_df.groupby("strategy_name").agg({"equity": ["mean", "median", "std", "min", "max"], "sharpe": ["mean", "median", "std", "min", "max"], "max_dd": ["mean", "median", "std", "min", "max"]})
